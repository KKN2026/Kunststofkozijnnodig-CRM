  ...
  return (
    <>
      <button
        ...
        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-gradient-to-r from-[#1e40af] to-[#3b82f6] text-white text-sm font-medium hover:from-[#1e3a8a] hover:to-[#1e40af] shadow-sm"
        title="AI-assistent"
      >
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex justify-end" onClick={(e) => { if (e.target === e.currentTarget) setOpen(false) }}>
          ...
          <div className="relative bg-white w-full max-w-md h-full flex flex-col shadow-2xl">
            <div className="px-4 py-3 border-b border-gray-200 flex items-center justify-between bg-gradient-to-r from-[#1e40af] to-[#3b82f6] text-white">
                  ...
                  <div className="space-y-1.5">
                    {VOORBEELD_VRAGEN.map(v => (
                      <button
                        ...
                        type="button"
                        onClick={() => sendVraag(v)}
                        className="block w-full text-left px-3 py-2 text-xs bg-white border border-gray-200 rounded-lg hover:border-[#1e40af] hover:bg-green-50 transition-colors"
                      >
                        {v}
                      </button>
                    ))}
            ...
            >
              <input
                ...
                disabled={bezig}
                className="flex-1 px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1e40af] focus:border-transparent disabled:bg-gray-50"
              />
              <button
                type="submit"
                disabled={bezig || !vraag.trim()}
                className="p-2 bg-[#1e40af] text-white rounded-lg hover:bg-[#1e3a8a] disabled:opacity-50 disabled:cursor-not-allowed"
              >

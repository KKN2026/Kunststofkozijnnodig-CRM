...
export async function getFactuurEmailDefaults(factuurId: string) {
  ...
  let medewerkerNaam = 'Kunststofkozijnnodig.nl'
  if (user) {
  ...
  // Permanente Kunststofkozijnnodig.nl-URL: redirect naar de actuele Mollie-link, of genereert
...
${medewerkerNaam}`
  ...
  return {
    ...
    subject: `Factuur ${factuur.factuurnummer} - Kunststofkozijnnodig.nl`,
    body,
...
export async function sendFactuurEmail(factuurId: string, options: {
  ...
  try {
    ...
    await sendEmail({
      ...
      fromName: mwInfo?.naam || 'Kunststofkozijnnodig.nl',
...
export async function syncSnelstartBetalingen(administratieIdOverride?: string) {
  ...
  for (const f of crmFacturen) {
    ...
    // deze guard draait deze sync elke Mollie-afboeking terug naar 'verzonden':
    // de klant krijgt wél netjes een betalingsbevestiging, maar in het CRM blijft
    // de factuur openstaan — inclusief aanmaningen en een verkeerd debiteuren-
    // saldo. (Augustus 2026 in het Rebu-CRM: zo bleven 6 facturen à € 28.630,71
    // onterecht open. Hier preventief toegepast, zelfde code.)
...
export async function verlengAanvraagSla(taakId: string) {
  ...
  if (rel?.email) {
    try {
      ...
      await sendEmail({
        to: rel.email,
        subject: 'Uw offerte-aanvraag bij Kunststofkozijnnodig.nl',
...
<p>Met vriendelijke groet,<br/>Kunststofkozijnnodig.nl</p>`,
      })
      mailVerstuurd = true
    } catch (err) {
...
Er is een account voor u aangemaakt bij Kunststofkozijnnodig.nl.
...
Wij raden u aan uw wachtwoord na de eerste login te wijzigen.`
    ...
    try {
      await sendEmail({
        to: email,
        subject: 'Uw account — Kunststofkozijnnodig.nl',
...
${medewerkerNaam}`
  ...
  const subject = projectNaam
    ...
    : `Offerte ${offerte.offertenummer} - Kunststofkozijnnodig.nl`
...
export async function sendOfferteEmail(offerteId: string, options: {
  ...
  try {
    ...
    await sendEmail({
      ...
      fromName: medewerkerInfo?.naam || 'Kunststofkozijnnodig.nl',
...
export async function resendEmailLog(emailLogId: string, overrideTo?: string) {
  ...
  try {
    await sendEmail({
      ...
      html: log.body_html,
      attachments,
      fromName: profiel?.naam || 'Kunststofkozijnnodig.nl',
...
export async function getDeliveryEmailDefaults(orderId: string) {
  ...
  return {
    to: rel?.email || '',
    subject: `Leverplanning ${order.ordernummer} - Kunststofkozijnnodig.nl`,
...
Er is een klantenportaal account voor u aangemaakt bij Kunststofkozijnnodig.nl. Via het portaal kunt u uw offertes, orders en berichten bekijken.
...
Wij raden u aan uw wachtwoord na de eerste login te wijzigen via de instellingen in het portaal.`
  ...
  try {
    await sendEmail({
      to: data.email,
      subject: 'Uw klantenportaal account — Kunststofkozijnnodig.nl',
...
export async function stuurVakantieVooraankondiging(vrijeDagId: string) {
  ...
  const infoMail = admin?.email || process.env.SMTP_FROM || 'info@kunststofkozijnnodig.nl'
  ...
  for (const k of klanten) {
    ...
    try {
      await sendEmail({
        ...
        subject: 'Even wat minder snel bereikbaar — Kunststofkozijnnodig.nl',
        html: `<p>Beste ${aanhef},</p>
...
Er is een medewerker-account voor u aangemaakt bij Kunststofkozijnnodig.nl. Via het dashboard kunt u uw taken, planning en uren bekijken.
...
Wij raden u aan uw wachtwoord na de eerste login te wijzigen.`
    ...
    try {
      await sendEmail({
        to: email,
        subject: 'Uw medewerker-account — Kunststofkozijnnodig.nl',

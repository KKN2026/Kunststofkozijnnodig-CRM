                  ...
                  return (
                    <Link key={i} href={`/facturatie?periode=${jaar}-${String(i + 1).padStart(2, '0')}`} title={`Bekijk de facturen van ${m.naam} ${jaar}`} className="flex-1 flex flex-col items-center gap-1 group relative">
                      ...
                      <div className="w-full flex flex-col items-center justify-end h-44">
                        ...
                        <div
                          className={`w-full max-w-[48px] rounded-t-md transition-all ${isHuidig ? 'bg-[#1e40af]' : 'bg-[#1e40af]/30 group-hover:bg-[#1e40af]/60'} ${m.omzet === 0 ? 'bg-gray-100' : ''}`}
                          style={{ height: `${Math.max(hoogte, m.omzet > 0 ? 3 : 1)}%` }}
                        />
                      </div>
                      <span className={`text-[10px] ${isHuidig ? 'font-bold text-[#1e40af]' : 'text-gray-500'}`}>{m.naam}</span>
                    </Link>
                  )
                })}
              </div>
                    ...
                    {maandData.map((m, i) => {
                      ...
                      return (
                        <tr key={i} className={`border-b border-gray-100 ${isHuidig ? 'bg-blue-50 font-medium' : ''}`}>
                          <td className="py-2">
                            <Link href={periodeHref} className="hover:text-[#1e40af] hover:underline">{m.naam} {jaar}</Link>
                          </td>
                          <td className="py-2 text-right">
                            <Link href={periodeHref} title={`Bekijk de ${m.aantal} facturen van ${m.naam} ${jaar}`} className="hover:text-[#1e40af] hover:underline">{m.aantal}</Link>
                          </td>

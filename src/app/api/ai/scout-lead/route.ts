...
const leadSchema = z.object({
  ...
  relevantie_score: z.number().describe('0-10: hoe relevant voor Kunststofkozijnnodig.nl (kunststof/aluminium kozijnen). 0=irrelevant, 10=perfecte fit').default(5),
...
export async function POST(req: NextRequest) {
  ...
  try {
    const { object } = await generateObject({
      ...
      system: `Je bent een lead-scout voor Kunststofkozijnnodig.nl (Wormerveer, regio Zaanstreek). Kunststofkozijnnodig.nl levert kunststof + aluminium kozijnen, ramen, deuren en schuifpuien aan particulieren EN aannemers. Je scant gepaste tekst (Marktplaats, Werkspot, Facebook, email) en extraheert mogelijke leads.

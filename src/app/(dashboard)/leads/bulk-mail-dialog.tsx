...
export function BulkMailDialog({ open, onClose, leads, onSent }: { open: boolean; onClose: () => void; leads: Lead[]; onSent: (aantal: number) => void }) {
  ...
  const [onderwerp, setOnderwerp] = useState('Kennismaking Kunststofkozijnnodig.nl')
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <div className="flex gap-2">
            <button onClick={() => setTemplate('eerste_contact')} className={`flex-1 px-4 py-3 rounded-lg border text-left transition-colors ${template === 'eerste_contact' ? 'border-[#1e40af] bg-[#1e40af]/5' : 'border-gray-200 hover:border-gray-300'}`}>
            ...
            <button onClick={() => setTemplate('na_bellen')} className={`flex-1 px-4 py-3 rounded-lg border text-left transition-colors ${template === 'na_bellen' ? 'border-[#1e40af] bg-[#1e40af]/5' : 'border-gray-200 hover:border-gray-300'}`}>
        ...
        <div className="flex items-end gap-2">
          ...
          <Button onClick={handleGenereer} disabled={genereren} className="bg-[#1e40af] text-white hover:bg-[#1e3a8a]">
        ...
        <div>
          ...
          <textarea
            ...
            className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm font-sans focus:outline-none focus:ring-2 focus:ring-[#1e40af]"
          />
        </div>
